...
unsigned int ipu_isys_mbus_code_to_data_type(u32 code)
{
	switch (code) {
	case MEDIA_BUS_FMT_RGB565_1X16:
				  ...
				  unsigned int pad, unsigned int which)
{
	...
	for (i = 0; i < sd->entity.num_pads; i++) {
		...
		crops[i] = __ipu_isys_get_selection(sd, state,
						    V4L2_SEL_TGT_CROP, i,
						    which);
		compose[i] = __ipu_isys_get_selection(sd, state,
						      V4L2_SEL_TGT_COMPOSE, i,
						      which);
	}
	...
	case IPU_ISYS_SUBDEV_PROP_TGT_SINK_FMT:
		...
		ret = ipu_isys_subdev_fmt_propagate(sd, state, ffmt,
						    crops[pad], tgt + 1, pad,
						    which);
		goto out_subdev_fmt_propagate;
	...
	case IPU_ISYS_SUBDEV_PROP_TGT_SINK_COMPOSE:
		...
		for (i = 1; i < sd->entity.num_pads; i++) {
			...
			ret = ipu_isys_subdev_fmt_propagate(sd, state,
							    ffmt, compose[i],
							    tgt + 1, i, which);
			if (ret)
				goto out_subdev_fmt_propagate;
				  ...
				  struct v4l2_subdev_format *sink_fmt)
{
	...
	if (strncmp(source_sd->name, IPU_ISYS_ENTITY_PREFIX,
		...
		dev_dbg(&asd->isys->adev->dev, "%s: using source %d\n",
			sd->entity.name, stream->source);
	} else if (source_sd->entity.num_pads == 1) {
		/* All internal sources have a single pad. */
		stream->external = link->source;
		stream->source = to_ipu_isys_subdev(source_sd)->source;
