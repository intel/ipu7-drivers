...
static int ipu_isys_buf_prepare(struct vb2_buffer *vb)
{
...
static void buf_finish(struct vb2_buffer *vb)
{
	...
	dev_dbg(&av->isys->adev->dev, "%s buffer finish\n", av->vdev.name);
}
...
static int ipu_isys_stream_start(struct ipu_isys_video *av,
				 struct ipu_isys_buffer_list *bl, bool error)
{
	...
	ret = ipu_isys_video_set_streaming(av, 1, bl);
	...
	do {
		...
		ipu_fw_isys_dump_frame_buff_set(&stream->isys->adev->dev, buf,
						stream->nr_output_pins);

		ipu_isys_buffer_list_queue(bl,
					   IPU_ISYS_BUFFER_LIST_FL_ACTIVE, 0);

		ret = ipu_fw_isys_complex_cmd(stream->isys,
...
static void buf_queue(struct vb2_buffer *vb)
{
	...
	if (ret < 0) {
		if (ret == -EINVAL) {
			dev_err(dev, "error: should not happen\n");
			WARN_ON(1);
		} else {
			dev_dbg(dev, "No enough buffers available\n");
		}
		goto out;
	}
	...
	msg = ipu_get_fw_msg_buf(stream);
	if (!msg) {
		dev_err(dev, "Get fw msg from list failed\n");
		ret = -ENOMEM;
		goto out;
	}
	buf = to_frame_msg_buf(msg);

	ipu_isys_buffer_to_fw_frame_buff(buf, stream, &bl, 0);
	...
	if (!stream->streaming) {
		...
		ret = ipu_isys_stream_start(av, &bl, true);
		if (ret)
...
static int start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	/* every ipu_isys_stream is only enabled once */
	av->stream = ipu_isys_get_stream(av->isys);
	if (!av->stream) {
	...
	if (!stream->nr_streaming) {
		first = true;
		ret = ipu_isys_video_prepare_streaming(av);
		if (ret) {
			mutex_unlock(&stream->mutex);
	...
	ret = ipu_isys_stream_start(av, bl, false);
	if (ret)
		goto out_stream_start;
			      ...
			      struct ipu_insys_resp *info)
{
	...
	dev_dbg(&isys->adev->dev, "buffer: %s: received buffer %8.8x %d\n",
...
static const struct vb2_ops ipu_isys_queue_ops = {
